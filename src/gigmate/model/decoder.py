...
class Decoder(nn.Module):
    ...
    def forward(self, x: Tensor, cross_attention_src: Tensor, sequence_lengths: Tensor, use_cache: bool = False, cache: Optional[List[Tensor]] = None, cache_index: Optional[int] = None) -> Tuple[Tensor, Optional[List[Tensor]]]:
        ...
        for i, layer in enumerate(list(self.transformer_layers)):
            ...
            x, updated_layer_cache = layer(x, cross_attention_src=cross_attention_src, sequence_lengths=sequence_lengths, use_cache=use_cache, cache=layer_cache, cache_index=cache_index)
