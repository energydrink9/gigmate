...
class TransformerBlock(nn.Module):
    ...
    def forward(
            ...
            use_cache: bool = False, cache: Optional[Tensor] = None,
            cache_index: Optional[int] = None,            
    ) -> Tuple[Tensor, Optional[Tensor]]:

        norm1 = self.layernorm1(x)
        tgt2, cache = self.self_attention(norm1, norm1, norm1, use_cache=use_cache, cache=cache, cache_index=cache_index, sequence_lengths=sequence_lengths)
        ...
        if self.has_cross_attention is True:
            tgt2, cache = self.cross_attention(tgt2, cross_attention_src, cross_attention_src, use_cache=use_cache, cache=cache, cache_index=cache_index, sequence_lengths=sequence_lengths)
            torch.isnan(tgt2).any()  # Seems to prevent nans from propagating. TODO: understand why
