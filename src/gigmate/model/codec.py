...
def bundle_chunks_and_add_special_tokens(chunks: List[torch.Tensor], encoded_tokens_per_chunk: int, add_start_and_end_token: bool) -> List[torch.Tensor]:
...
def encode(audio: torch.Tensor, sr: int, device: Device, add_start_and_end_tokens: bool = False) -> Tuple[List[torch.Tensor], float]:
    ...
    encoded_chunks_bundles = bundle_chunks_and_add_special_tokens(encoded_chunks, encoded_tokens_per_chunk, add_start_and_end_tokens)
